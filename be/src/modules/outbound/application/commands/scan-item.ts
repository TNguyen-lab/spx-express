...
export async function scanItem(id: string, productId: string, barcode: string, role: string, userId: string) {
  ...
  const payload = { outboundNumber: outbound.outboundNumber, productId, barcode };
  await publishEvent(OutboundEvents.Scanned, 'Outbound', outbound.id, payload, userId);
