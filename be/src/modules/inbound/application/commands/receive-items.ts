...
export async function receiveItems(id: string, role: InboundRole, userId: string) {
  return withTransaction(async (tx) => {
    const inbound = await getInboundOrThrow(id, tx);
    validateTransition(inbound.status as never, InboundStatus.ITEMS_RECEIVED as never, role);
    const updated = await tx.inbound.update({ where: { id }, data: { status: 'ITEMS_RECEIVED', receivedDate: new Date() } });
    await publishEventWithTx(tx, InboundEvents.Received, 'Inbound', inbound.id, { inboundNumber: inbound.inboundNumber }, userId);
    return updated;
  });
}
