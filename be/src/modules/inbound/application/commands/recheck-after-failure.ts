...
export async function recheckAfterFailure(id: string, role: InboundRole, userId: string) {
  return withTransaction(async (tx) => {
    ...
    await publishEventWithTx(tx, InboundEvents.QualityChecking, 'Inbound', inbound.id, { inboundNumber: inbound.inboundNumber, recheck: true }, userId);
    return updated;
  });
}
