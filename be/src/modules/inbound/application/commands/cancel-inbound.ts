...
export async function cancelInbound(id: string, role: InboundRole, userId: string, reason?: string) {
  return withTransaction(async (tx) => {
    const inbound = await getInboundOrThrow(id, tx);
    validateTransition(inbound.status as never, InboundStatus.INBOUND_CANCELLED as never, role);
    const updated = await tx.inbound.update({ where: { id }, data: { status: 'INBOUND_CANCELLED', notes: reason || inbound.notes } });
    await publishEventWithTx(tx, InboundEvents.Cancelled, 'Inbound', inbound.id, { inboundNumber: inbound.inboundNumber, reason }, userId);
    return updated;
  });
}
